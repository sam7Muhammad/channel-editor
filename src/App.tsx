...
export const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col w-full selection:bg-cyan-500/30 selection:text-cyan-200 relative">
      ...
      {/* Main Content Area */}
      <main className="flex-1 flex flex-col w-full min-h-[calc(100vh-80px)] p-1">
        {channels.length === 0 ? (
          <FileUploader
            onFileSelected={handleFileSelected}
            isLoading={isLoading}
          ...
          <div className="flex-1 flex flex-col w-full space-y-2">
            ...
            <div className="flex-1 flex flex-col w-full rounded-xl overflow-hidden glass border border-slate-800 shadow-xl">
              <FilterBar
                filter={filter}
